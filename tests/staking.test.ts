...
test<LucidContext>("Staking - Create Position", async ({
    ...
    await runAndAwaitTx(lucid,StakingContract.openPosition(1_000_000n, systemParams, lucid));
});
...
test<LucidContext>("Staking - Adjust Position", async ({
    ...
    const myStakingPosition = await findStakingPosition(
        ...
        {
            currencySymbol: systemParams.stakingParams.stakingToken[0].unCurrencySymbol,
            tokenName: fromText(systemParams.stakingParams.stakingToken[1].unTokenName),
        }, 
        pkh.hash
    );

    await runAndAwaitTx(lucid, StakingContract.adjustPosition(myStakingPosition, 1_000_000n, systemParams, lucid));
});
...
test<LucidContext>("Staking - Close Position", async ({
    ...
    await runAndAwaitTx(lucid, StakingContract.closePosition(myStakingPosition, systemParams, lucid));
});